#!/bin/bash
# Filename:    Rocky_Linux_9_Install_Redis.sh
# Revision:    1.0
# Date:        2025/07/18
# Author:      star
# Email:       star@xgss.net
...
# 定义变量
redis_software="/data/software/"	#redis软件目录
redis_port="63920"						#redis端口
redis_conf="/data/conf/"			    #redis配置所在目录	
redis_dir="/data/redis/${redis_port}"	#redis持久化目录
redis_passwd="YpassWord666"
redis_size="1024mb"  # 可以设置为0，Redis 使用无限制内存，可导致系统 OOM
...
wget http://js.funet8.com/centos_software/redis-7.4.1.tar.gz
...
# 配置redis
mkdir /etc/init.d/
cp ${redis_software}redis-7.4.1/utils/redis_init_script /etc/init.d/redis_${redis_port}
...
#创建目录
mkdir -p ${redis_conf}
cp /data/software/redis-7.4.1/redis.conf  ${redis_conf}${redis_port}.conf
...
loglevel notice
#日志文件的位置 
logfile "${redis_dir}/redis_${redis_port}.log"
#设置数据库的数目
databases 16
...
# 连接redis
# redis-cli -h 127.0.0.1 -p ${redis_port} -a ${redis_passwd}

# 关闭redis
# redis-cli -h 127.0.0.1 -p ${redis_port} -a ${redis_passwd} shutdown
...
#logfile "/data/redis/63921/redis_63921.log"
